...
public class MediaHandlerDemo {
	// 运行前先设置好以下三个参数
	private static final String BUCKET_NAME = "streamd";
	private static final String OPERATOR_NAME = "mupengfei	";
	private static final String OPERATOR_PWD = "mu789456";

	public static void main(String[] args) {
//		String[] ids = testMediaProcess();
		String[] ids = new String[1];
		ids[0] = "72c194f2834823be54800dffcbb460cb";
		if (ids != null) {
			testMediaStatus(ids);
			testMediaResult(ids);
		}
	...
	private static String[] testMediaProcess() {
		...
		// 初始化 MediaHandler
		MediaHandler handler = new MediaHandler("streamd", "mupengfei", "mu789456");
//		MediaHandler handler = new MediaHandler(BUCKET_NAME, OPERATOR_NAME, OPERATOR_PWD);
		...
		// 需要处理视频路径
		paramsMap.put(MediaHandler.Params.SOURCE, "/test/3.mov");
		...
		json.put(MediaHandler.Params.AVOPTS, "/vb/5000/s/1920*1080/as/1/r/24");
		...
		json.put(MediaHandler.Params.SAVE_AS, "31080.mp4");
...
//		array.put(json2);
	...
	private static void testMediaStatus(String[] ids) {
		...
		try {
			Result result = handler.getStatus(paramsMap);
	...
	private static void testMediaResult(String[] ids) {
		...
		try {
			Result result = handler.getResult(paramsMap);
