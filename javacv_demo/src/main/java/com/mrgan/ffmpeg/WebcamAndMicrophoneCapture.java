...
public class WebcamAndMicrophoneCapture {
	...
	public static void main(String[] args) throws Exception, org.bytedeco.javacv.FrameGrabber.Exception {
		...
		final FFmpegFrameRecorder recorder = new FFmpegFrameRecorder(
				"rtmp://my-streaming-server/app_name_here/instance_name/stream_name", captureWidth, captureHeight, 2);
		...
		while ((capturedFrame = grabber.grab()) != null) {
			if (cFrame.isVisible()) {
				new Thread(new Runnable() {
					public void run() {
						// Pick a format...
						// NOTE: It is better to enumerate the formats that the
						// system supports,
						// because getLine() can error out with any particular
						// format...
						// For us: 44.1 sample rate, 16 bits, stereo, signed,
						// little endian
						...
						try {
							...
							// It's possible to have more control over the
							// chosen audio device with this line:
							// TargetDataLine line =
							...
							// Using a ScheduledThreadPoolExecutor vs a while
							// loop with
							// a Thread.sleep will allow
							// us to get around some OS specific timing issues,
							// and keep
							// to a more precise
							// clock as the fixed rate accounts for garbage
							// collection
							// time, etc
							// a similar approach could be used for the webcam
							// capture
							// as well, if you wish
